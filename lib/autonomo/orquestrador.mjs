...
async function executarFase({ faseId, tasksDir, taskId, repoRoot, manifesto, opcoes }) {
  ...
  const ctx = ISO.materializar({ tasksDir, taskId, faseId, repoRoot });
  log(`\n▶ Fase ${faseId} — ${fase.nome}`);
  if (ctx.noRepo) {
    log(`   cwd:      ${ctx.dir}  ← roda no repositório`);
  } else {
  ...
  try {
    if (ctx.agente) {
      ...
      const recolheu = ISO.recolher({ dir: ctx.dir, tasksDir, taskId, artefato: fase.artefato, noRepo: ctx.noRepo });

      // Fase 2 sem artefato não é falha técnica: é o requirements-analyst
      // exercendo o gate — a task não tem informação suficiente. O destino é
      // `ia-aguardando-info` (dono: quem escreveu a tarefa), não
      // `ia-bloqueada` (dono: o dev). Misturar os dois destrói a informação
      // mais valiosa que o pipeline produz: o problema está nas tarefas ou no
      // pipeline?
      // Fase 1 depende do conector ClickUp, que é OAuth do claude.ai e vive na
      // sessão interativa — não em subprocessos `claude -p`. Não há como um
      // processo não-interativo completar aquele fluxo de autenticação.
      //
      // Em vez de falhar como se fosse problema do agente, o orquestrador diz
      // o que fazer: rodar `/ler-task <ID>` na sessão interativa uma vez, e
      // então retomar. As fases seguintes não precisam do ClickUp até a 10.
      if (!recolheu && faseId === '1') {
        M.falharFase(manifesto, faseId, 'sem acesso ao ClickUp');
        M.salvar(tasksDir, taskId, manifesto);
        log('   ⚠ o conector ClickUp não existe em subprocesso não-interativo');
        return {
          parar: true,
          motivo:
            `a fase 1 precisa do conector ClickUp, que só existe na sessão interativa.
` +
            `  Rode uma vez na sessão do Claude Code:  /ler-task ${taskId}
` +
            `  Depois retome:  pipeline run ${taskId}`,
          statusClickUp: null,
        };
      }

      if (!recolheu && faseId === '2') {
        const faltantes = (resultado.saida || '').slice(0, 1200);
        writeFileSync(
          join(tasksDir, taskId, '02-requirements.md'),
          `# Análise de Requisitos — REPROVADA

${faltantes}
`
        );
        M.falharFase(manifesto, faseId, 'task incompleta');
        M.salvar(tasksDir, taskId, manifesto);
        R.registrar(tasksDir, taskId, {
          fase: faseId, agente: ctx.agente, duracao_ms: Date.now() - inicio,
          custo_usd: resultado.custoUsd, status: 'gate_reprovou',
          leu: ctx.materializados, negados: ctx.negados,
          observacao: 'task incompleta — falta informação',
        });
        log('   ⚠ task incompleta — o que falta ficou em 02-requirements.md');
        return {
          parar: true,
          motivo: 'task incompleta: o analista de requisitos pediu mais informação',
          statusClickUp: 'ia-aguardando-info',
        };
      }
    }
    ...
    // NUNCA limpar quando a fase roda no repo — ctx.dir É o repositório.
    if (!opcoes.manterDirs && !ctx.noRepo) ISO.limpar(ctx.dir);
  }
}
